...
function handleHash_(o,v) {
    ...
    if(!optionStr){
        ...
        $.each(optionArr,function(i,itemStr){
            ...
            if(itemArr[0] == 'tel' || itemArr[0] == 'phone' || itemArr[0] == 'email' || itemArr[0] == 'website' || itemArr[0] == 'gwebsite' || itemArr[0] == 'mark' || itemArr[0] == 'patent' || itemArr[0] == 'finance' || itemArr[0] == 'listed' || itemArr[0] == 'shixin' || itemArr[0] == 'zzq' || itemArr[0] == 'rjzzq' || itemArr[0] == 'insured'){
                tel_email++;
            }
    ...
    if(o == 'tel' || o == 'phone' || o == 'email' || o == 'website' || o == 'gwebsite' || o == 'mark' || o == 'patent' || o == 'finance' || o == 'listed' || o == 'shixin' || o == 'zzq' || o == 'rjzzq' || o == 'insured'){
        if(tel_email == 11){
...
function updateDesc(type){
    ...
    var hideFlag = true;//默认隐藏
    if(type == 1){
        var arr = ['index','searchType','coyType','statusCode','registfund','startDate','province','city','county','industrycode','subindustrycode','thirdindustrycode','tel','phone','email','website','gwebsite','mark','patent','finance','listed','shixin','zzq','rjzzq','insured'];
    }else if(type == 2){
...
function addSearchIndex(search_key,search_index) {
    $.post(INDEX_URL+'search_addSearchIndex',{search_key:search_key,search_index:search_index});
}
...
function dateAgo(timestamp) {
    ...
    if($c<86400) return (($c-$c%3600)/3600)+'时'+parseInt(($c%3600)/60)+'分钟前';
    if($c<1296000) return (($c-$c%86400)/86400)+'天前';
    if($c>=1296000) return moment(timestamp*1000).format('YYYY-MM-DD');
}
