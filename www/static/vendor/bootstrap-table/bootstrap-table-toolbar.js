...
!function($) {
    ...
    var showAvdSearch = function(pColumns, searchTitle, searchText, that) {
        if (!$("#avdSearchModal" + "_" + that.options.idTable).hasClass("modal")) {
            ...
            vModal += sprintf("   <h4 class=\"modal-title\">%s</h4>", "搜索");
            vModal += "  </div>";
            ...
            $('#' + that.options.idForm + " [id*=btnCloseAvd]")
                .off("click")
                .on("click",function (event) {
                    window.searchParams = $(this).closest("form").serialize()
                    that.onColumnAdvancedSearch(event)
                });
    ...
    var createFormAvd = function(pColumns, searchText, that) {
        ...
        $.each(that.options.searchForm, function (i,vObjCol) {
        ...
        htmlForm.push(sprintf('<button type="button" id="btnCloseAvd%s" class="btn btn-primary"><i class="glyphicon glyphicon-search"></i> %s</button>', "_" + that.options.idTable, "搜索"));
        htmlForm.push('</div>');
        htmlForm.push('</form>');
    ...
    BootstrapTable.prototype.initToolbar = function() {
        ...
        this.$toolbar.find("div.search").hide()
    };
