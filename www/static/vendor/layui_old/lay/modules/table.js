...
layui.define(["laytpl", "laypage", "layer", "form"], function(e) {
	...
	var t = layui.$,
		...
		F = function(e) {
			...
			}(), '<th data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} ' + t + ' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>', '<div class="layui-table-cell laytable-cell-', "{{# if(item2.colspan > 1){ }}", "group", "{{# } else { }}", "{{d.index}}-{{item2.field || i2}}", '{{# if(item2.type !== "normal"){ }}', " laytable-cell-{{ item2.type }}", "{{# } }}", "{{# } }}", '" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>', '{{# if(item2.type === "checkbox"){ }}', '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>', "{{# } else { }}",
			'{{# if(item2.pos){ }}',
			'<a data-fixed="{{item2.pos}}" data-field="{{item2.field || i2}}" class="switch-fixed" href="javascript:;">{{item2.title||""}}</a>',
			'{{# }else{ }}',
			'<span>{{item2.title||""}}</span>',
			'{{# } }}',
			"{{# if(!(item2.colspan > 1) && item2.sort){ }}", '<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc"></i><i class="layui-edge layui-table-sort-desc"></i></span>', "{{# } }}", "{{# } }}", "</div>", "</th>", e.fixed ? "{{# }; }}" : "", "{{# }); }}", "</tr>", "{{# }); }}", "</thead>", "</table>"].join("")
		},
	...
	M.prototype.config = {
		...
		e.pullData(e.page, e.loading()), e.events()
    ...
    M.prototype.fetchResponse = function(res, key){
    ...
    M.prototype.pullData = function(e, i) {
		...
		if (a.startTime = (new Date).getTime(), n.url) {
            var c = {};
            // !!!!!!!!!!!!!!!!!!!!
			c[o.pageName] = e-1, c[o.limitName] = n.limit;
			if(n.urlType == "disk"){
				c[o.pageName] = e, c[o.limitName] = n.limit;
			}
            var s = t.extend(c, n.where);
			n.contentType && 0 == n.contentType.indexOf("application/json") && (s = JSON.stringify(s)), SuperPost({
				urlType: n.urlType || "self",
				type: n.method || "get",
				url: n.url,
				...
				success: function(t) {
					// !!!!!!!!!!!!!!!!!!!!
					if (typeof n.responseHandler == "function") {
						t = n.responseHandler(t);
						if(/\/api\/search\/accloan/.test(n.url) && t.success && n.autoShift !== false){
							translateOdsInData(t.data.content || t.data);
						}
					}
                    if(a.fetchResponse(t, r.statusName) != r.statusCode){
                        a.renderForm();
                        return a.layMain.html('<div class="'+ f +'">'+ (a.fetchResponse(t, r.msgName) || '返回的数据状态异常') +'</div>');
                      }
                      a.renderData(t, e, a.fetchResponse(t, r.countName)), d();
                      i && l.close(i);
                      typeof n.done === 'function' && n.done(t, e, a.fetchResponse(t, r.countName));
				},
				error: function(e, t) {
					console.log(e, t);
		...
		var c = this,
			s = c.config,
			u = c.fetchResponse(e, s.response.dataName)  || [], // !!!!!!!!!!!!!
			y = [],
			p = [],
			m = [],
			v = function() {
	...
	// !!!!!!!!!!!
	// 切换固定列
	t(document).on('click', '.switch-fixed', function(){
		var that = t(this);
		var fixed = that.data('fixed');
		var field = that.data('field')
		if(fixed.indexOf('no')>=0){
			location.href = putUrlParam(field, fixed.substr(2));
		}else{
			location.href = putUrlParam(field, 'no'+fixed);
		}
	});
