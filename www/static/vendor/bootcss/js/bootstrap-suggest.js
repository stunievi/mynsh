...
(function (factory) {
    ...
    function refreshDropMenu($input, data, options) {
        ...
        if(window.beforeSuggest){
            console.log(data)
            window.beforeSuggest(data.value);
        }
     ...
     */
    function isInWord(keyword, key, value, options,k) {
        value = $.trim(value);
                ...
                if (keyword) {
                    ...
                    for (i = 0; i < len; i++) {
                        for (key in data.value[i]) {
                            if (
                                data.value[i][key] &&
                                isInWord(keyword, key, data.value[i][key] + '', options,k)
                            ) {
