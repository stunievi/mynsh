...
;layui.define(["layer", "laytpl", "upload"], function (i) {
    ...
    var u = function (i) {
        ...
        h = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }} data-time="{{d.timestamp || 0}}"  >', '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}", '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', "</li>"].join(""),
    ...
    var x, b, w, k, C, S = function (i, a, n) {
        ...
        if (i.content = a.textarea.val(), "" !== i.content.replace(/\s/g, "")) {
            ...
            //只允许有一个
            if(d.sendMessage.length){
                var callback = function () {
                    e.append(n(h).render(i));
                    e.parent().scrollTop(10000)
                }
                d.sendMessage[0] && d.sendMessage[0](s,callback)
            }
        ...
        if (w) {
            ...
            i.system ? l !== -1 && c.append('<li class="layim-chat-system"><span>' + i.content + "</span></li>") : "" !== i.content.replace(/\s/g, "") && (function() {
                if(i.cid){
                    if(c.find("li[data-cid='"+i.cid+"']").length == 0){
                        c.append(n(h).render(i))
                    }
                }

                c.append(c.find("li[data-cid]").toArray().sort(function (a,b) {
        ...
        layui.each(e[a.data.type + a.data.id], function (i, a) {
            ...
            if(a.cid){
               if(t.find("li[data-cid='"+a.cid+"']").length == 0){
                   t.append(n(h).render(a))
               }
            }
            else{
