...
public interface IGlobalPermissionDao extends JpaRepository<GlobalPermission,Long>{
    ...
    class SQL{
        public static final String GET_UIDS = "select user.id from GlobalPermission gp, User user join user.quarters uq where (" +
            //按人授权,直接取uid
        ...
        public static final String GET_UIDS_WITHOUT_OID = "select user.id from GlobalPermission gp, User user join user.quarters uq where (" +
            //按人授权,直接取uid
            "(gp.userType = 2 and gp.linkId = user.id) or " +
            //按岗位授权,拥有这个岗位
            "(gp.userType = 1 and uq.id = gp.linkId) or " +
