...
<<<<<<< HEAD
     ...
     * @return
     */
    public Object getOneGao(SQLManager sqlManager, JSONObject object){
        Assert(S.notBlank(object.getString("accCode")));
        Map user = sqlManager.lambdaQuery(User.class)
                .andEq(User::getAccCode, object.getString("accCode"))
                .mapSingle();
        if (user == null) {
            user = C.newMap();
        }
        else{
            List<JSONObject> objects = sqlManager.select("user.查询组织机构列表(通过用户ID)", JSONObject.class, C.newMap("uid", user.get("id")));
            user.put("os", objects);
        }
        return user;
    }
     ...
     * @return
     */
    public Object getMethodsGao(SQLManager sqlManager, JSONObject object){
        String id = object.getString("id");
        Assert(S.notBlank(id));
