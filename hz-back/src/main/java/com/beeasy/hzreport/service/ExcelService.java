...
public class ExcelService {
    ...
    public File exportTableByTemplate(String tmpl, String dest, List<JSONObject> data){
        File file = new File(dest);
        try(
            FileOutputStream fos = new FileOutputStream(file);
            ){
            byte[] bytes = exportTableByTemplate2(tmpl,data);
            fos.write(bytes);
        }
        catch (IOException e){
            e.printStackTrace();
    ...
    public byte[] exportTableByTemplate(String tmpl, List<List<JSONObject>> datas) {
...
//        InputStream fis = resource.getInputStream();
    ...
    public byte[] exportTableByTemplate2(String tmpl, List<JSONObject> data) {
        List<List<JSONObject>> list = C.newList();
        list.add(data);
        return exportTableByTemplate(tmpl, list);
    }

    public byte[] exportExcelByTemplate(String tmpl, JSONObject head, List<JSONObject> data){
